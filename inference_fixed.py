...
HAR Inference Module (fixed)

Runs end-to-end inference using the processed UCI HAR resources.
"""
...
class FallDetector:
    def __init__(self, model_path="models/cnn_lstm_improved.keras", scaler_path="data/processed/har/scaler.pkl", encoder_path="data/processed/har/label_encoder.pkl"):
        ...
        # Define Attention layer locally to match the training-time custom layer
        ...
        # Expose Attention on the models module so loading finds 'models.Attention'
        try:
            models.Attention = Attention
        except Exception:
            pass

        # Load model with custom object mapping for Attention
        try:
            self.model = load_model(model_path, custom_objects={'Attention': Attention})
        except Exception:
            # fallback: attempt to import models module so that any custom classes are available
            try:
    ...
    def normalize(self, X):
        ...
        # Try to transform using feature names if available
        if hasattr(self.scaler, "feature_names_in_"):
            try:
                df = pd.DataFrame(X, columns=self.scaler.feature_names_in_)
    ...
    def create_windows(self, df, window_size=128, overlap=64):
        # Ensure expected sensor columns exist; if not, try common alternative names and map them.
        expected = self.sensor_columns()
        if all(col in df.columns for col in expected):
            working_df = df
        else:
            # common compact names used in some CSV exports
            compact = ["acc_x", "acc_y", "acc_z", "gyro_x", "gyro_y", "gyro_z"]
            if all(col in df.columns for col in compact):
                # map compact -> expected by duplicating accelerometer into body_acc
                ...
                # As a last resort, try to select numeric columns and pad/truncate to expected count
                numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
                if len(numeric_cols) >= len(expected):
                    working_df = df[numeric_cols[: len(expected)]]
        ...
        if n == 0:
            ...
            padded = np.vstack([sensor_data, padding])
    ...
    def predict(self, windows):
        ...
        labels = self.label_encoder.inverse_transform(predicted_index)
    ...
    def print_predictions(self, labels, confidence):
        ...
        for i in range(len(labels)):
            print(f"Window {i+1:03d} | Activity : {labels[i]:<5} | Confidence : {confidence[i]*100:.2f}%")
        print("=" * 70)
    ...
    def run(self, csv_path):
        ...
        labels, confidence, probabilities = self.predict(windows)
        self.print_predictions(labels, confidence)
        prediction = self.final_prediction(labels)
        print("\nInference Completed Successfully")
        return prediction, labels, confidence


def main():
    # Defaults target the processed HAR resources
